...
def remove_common_files(list1, list2):
	'''
	returns a subset of list1 that has common elements removed that were found in both lists
	or in other words - returns a subset of list1 that has elements unique to only list1
	'''
	new_list1 = list(set(list1) - set(list2))		# results in a list of values that are unique to list1
	new_list1.sort()
	return new_list1
...
def start_process(filenames, mode ):
	...
	for filename in filenames:
		try:
			...
			# getting a Movies object from the id that we got from the search results
			try:		# sometimes blank search results are returned
				tmdb_movie = tmdb.Movies(search.results[searchindex]['id'])		# Don't mind if any error is linted
			except IndexError:
				while len(search.results) == 0:		# Don't mind if any error is linted
					title = input('\nCould not find the movie, Enter alternate movie title >> ')
			...
			# setting the genres of the movie. I use ';' as a delimeter to searate the multiple genre values
			genre = ';'.join(imdb_movie.genres)
			# Going overboard and adding directors name to artist taf of the mp4 file
			...
			try:
				...
				try:
					...
					os.rename(newfilename[:-4] + 'new.mp4', 'auto fixed files\\' + newfilename[:-4] + '.mp4')
...
if not len(mp4_filenames) == 0:
	if not len(srt_filenames) == 0:
		mp4_with_srt_filenames = get_common_files(mp4_filenames, srt_filenames)
		if not len(mp4_with_srt_filenames) == 0:
			mp4_filenames = remove_common_files(mp4_filenames, mp4_with_srt_filenames)
...
if len(mp4_filenames) == 0 and len(mkv_filenames) == 0 and len(mp4_with_srt_filenames) == 0 and len(mkv_with_srt_filenames) == 0:
	print('There were no MP4 or MKV files found in the directory')
else:
	# Checks if there were any files that caused the Overflow Error, if yes then prints them out.
